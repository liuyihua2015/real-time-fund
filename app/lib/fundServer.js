...
function parsePingZhongSimpleFields(pzSource) {
  ...
  return {
    ...
    currentFundManagerDescription:
      pickString("fS_currentFundManagerDescription") || null,
...
async function fetchTencentStockChangePct(codes) {
  const list = Array.isArray(codes)
    ? codes.filter((c) => /^\d{6}$/.test(String(c)))
    : [];
...
export async function getFundDetail(code) {
  ...
  const assetAllocation = pzSource
    ? parsePingZhongAssetAllocation(pzSource)
    : [];
  ...
  return {
    ...
    managerDescription: pickFirstDefined(
      pz?.currentFundManagerDescription,
      null,
    ),
